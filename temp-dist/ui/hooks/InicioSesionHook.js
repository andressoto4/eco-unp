...
export var InicioSesionHook = function (maxAttempts, blockTime) {
    ...
    useEffect(function () {
        var interval = setInterval(verificarRenovarToken, 5 * 60 * 1000); // Llamar a la función de verificación y renovación del token periódicamente (por ejemplo, cada minuto)
    ...
    var decodeToken = function (token) {
        try {
            ...
            // Acceder a los datos
            var accessUrl = decoded.access_url;
            var accessUser = decoded.access_user;
            return { accessUrl: accessUrl, accessUser: accessUser };
        }
        catch (error) {
                    ...
                    return [4 /*yield*/, toast.promise(InicioSesionRequest(username, password, recaptchaToken), {
                            ...
                            success: {
                                render: function (_a) {
                                    ...
                                    var userToken = data.user_token;
                                    localStorage.setItem("access_token", accessToken);
                                    localStorage.setItem("user_token", userToken);
                                    var userInfo = decodeToken(userToken);
                                    if (userInfo && userInfo.accessUrl) {
                                        setTimeout(function () {
                                            // @ts-ignore
                                            var url = userInfo.accessUrl[0];
                                            navigate(url);
                                        }, 1000);
                                    }
                                    else {
    ...
    return { recaptchaToken: recaptchaToken, validated: validated, attempts: attempts, isBlocked: isBlocked, timer: timer, handleRecaptchaChange: handleRecaptchaChange, handleSubmit: handleSubmit, };
};
